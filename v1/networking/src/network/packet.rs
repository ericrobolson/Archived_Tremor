...
/// Only pure data of 1's and 0's are transmitted.
#[derive(Copy, Clone)]
pub struct Packet {
...
impl Packet {
    ...
    #[allow(unused_mut)]
    pub fn from_bytes(crc32: &Crc32, bytes: [u8; Self::TOTAL_PACKET_LEN]) -> Option<Self> {
        match decrypt_data(crc32, bytes) {
            Some(data) => {
    ...
    #[allow(unused_mut)]
    pub fn to_network_bytes(&self, crc32: &Crc32) -> [u8; Self::TOTAL_PACKET_LEN] {
        // Combine acks + data
...
fn hash_data(
    ...
    let checksum = crc32.hash(data.iter());
...
fn decrypt_data(
    ...
    let calculated_checksum = u32::from_le_bytes(crc32.hash(data.iter()));
